...
void QuickSort(int *base, int l, int r){
	...
	left = 0; right = n;
	while(right >= left){
		for(left++; (left<=right) && (base[left] <= base[pivot]);
		for(right--; (left<=right) && (base[right] > base[pivot]);;
		if(left <= right){
			int tmp = data[left];
			data[left] = data[rignt];
			data[right] = tmp;
		}
		else break;
	}
...
### 벨만포드
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ2NTUyNDIzOSwtODc4MDc4NTQ3LC05OT
Y0MDczMjIsNjEzMjA1NDYxLC0xOTM2MDA5NTUwLC0xMjE0OTIw
ODA1LDUzNTQ1MDA3NCwxOTk5OTM1OTAyLDg5OTI4Njc1Myw5ND
M3MjgyMywtMzkyMjU4NzMsMTIxODcxMTgxNywtMTE3MzcwNTMx
OCwyNDMxNDk3MjIsMjA4MDgxNTYxN119
-->